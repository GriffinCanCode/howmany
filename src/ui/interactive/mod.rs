...
impl InteractiveDisplay {
    ...
    pub fn show_results(
        ...
        if let Some(ref mut _modern) = self.modern_display {
            ...
            if let Ok(aggregated_stats) =
                stats_calculator.calculate_project_stats(stats, individual_files)
            {
                return self.show_comprehensive_results(&aggregated_stats, individual_files);
            }
        }
        self.legacy_display
            .show_results(stats, individual_files)
            .map_err(|e| crate::utils::errors::HowManyError::display(e.to_string()))
    }

    pub fn show_comprehensive_results(
        &mut self,
        ...
        if let Some(ref mut modern) = self.modern_display {
            ...
            modern
                .run_interactive_mode(code_stats, individual_files.to_vec())
                .map_err(|e| crate::utils::errors::HowManyError::display(e.to_string()))
        } else {
    ...
    fn show_enhanced_legacy_results(
        &mut self,
        aggregated_stats: &AggregatedStats,
        individual_files: &[(String, FileStats)],
    ) -> Result<()> {
        ...
        println!(
            "📏 Total Lines: {}",
            aggregated_stats.basic.total_lines.to_string().bright_blue()
        );
        println!(
            "💻 Code Lines: {}",
            aggregated_stats.basic.code_lines.to_string().bright_green()
        );
        println!(
            "💬 Comment Lines: {}",
            aggregated_stats
        ...
        println!(
            "📚 Documentation Lines: {}",
            aggregated_stats.basic.doc_lines.to_string().bright_cyan()
        );
        println!(
            "⬜ Blank Lines: {}",
            aggregated_stats
        ...
        println!(
            "💾 Total Size: {}",
            self.format_size_fallback(aggregated_stats.basic.total_size)
                .bright_cyan()
        );
