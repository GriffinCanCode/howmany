...
use crate::ui::interactive::charts::{
    render_advanced_language_visualizer, render_enhanced_overview,
};
...
pub fn render_search(f: &mut ratatui::Frame, area: Rect, app: &InteractiveApp) {
    ...
    let search_input = Paragraph::new(format!(
        "🔍 {} Search: {}",
        search_mode_text, app.search_state.query
    ))
    ...
    let stats_text = format!(
        "Found {} results out of {} files | Tab: cycle mode | Esc: exit | Enter: go to result",
        ...
        let items: Vec<ListItem> = app
            ...
            .map(|(i, result)| {
                ...
                ListItem::new(vec![
                    Line::from(vec![
                        Span::styled(
                            get_file_icon(&result.file_path),
                    ...
                    Line::from(vec![Span::styled(
                        format!(
                            "  {} | {} lines | {} code | Relevance: {}",
                            result.match_type, result.line_count, result.code_lines, relevance_bar
                        ),
...
pub fn render_overview(f: &mut ratatui::Frame, area: Rect, app: &InteractiveApp) {
    if let Some(ref stats) = app.stats {
        ...
        let aggregated_stats = stats_calculator
            .calculate_project_stats(stats, &app.individual_files)
            .unwrap_or_else(|_| {
...
fn create_aggregated_stats_from_basic(
    stats: &CodeStats,
) -> crate::core::stats::aggregation::AggregatedStats {
    ...
    use crate::core::stats::complexity::{
    ...
    let basic_stats = BasicStats {
        ...
        average_file_size: if stats.total_files > 0 {
            stats.total_size as f64 / stats.total_files as f64
        } else {
            0.0
        },
        average_lines_per_file: if stats.total_files > 0 {
            stats.total_lines as f64 / stats.total_files as f64
        } else {
            0.0
        },
        largest_file_size: stats
            .stats_by_extension
            .values()
            .map(|(_, file_stats)| file_stats.file_size)
            .max()
            .unwrap_or(0),
        smallest_file_size: stats
            .stats_by_extension
            .values()
            .map(|(_, file_stats)| file_stats.file_size)
            .min()
            .unwrap_or(0),
        stats_by_extension: stats
            .stats_by_extension
            .iter()
            .map(|(ext, (count, file_stats))| {
                (
                    ...
                    crate::core::stats::basic::ExtensionStats {
                        ...
                        average_lines_per_file: if *count > 0 {
                            file_stats.total_lines as f64 / *count as f64
                        } else {
                            0.0
                        },
                        average_size_per_file: if *count > 0 {
                            file_stats.file_size as f64 / *count as f64
                        } else {
                            0.0
                        },
                    },
    ...
    let ratio_stats = RatioStats {
        code_ratio: if stats.total_lines > 0 {
            stats.total_code_lines as f64 / stats.total_lines as f64
        } else {
            0.0
        },
        comment_ratio: if stats.total_lines > 0 {
            stats.total_comment_lines as f64 / stats.total_lines as f64
        } else {
            0.0
        },
        doc_ratio: if stats.total_lines > 0 {
            stats.total_doc_lines as f64 / stats.total_lines as f64
        } else {
            0.0
        },
        blank_ratio: if stats.total_lines > 0 {
            stats.total_blank_lines as f64 / stats.total_lines as f64
        } else {
            0.0
        },
        comment_to_code_ratio: if stats.total_code_lines > 0 {
            stats.total_comment_lines as f64 / stats.total_code_lines as f64
        } else {
            0.0
        },
        doc_to_code_ratio: if stats.total_code_lines > 0 {
            stats.total_doc_lines as f64 / stats.total_code_lines as f64
        } else {
            0.0
        },
...
pub fn render_main_stats(f: &mut ratatui::Frame, area: Rect, stats: &CodeStats) {
    ...
    let code_text = vec![
        ...
        Line::from(""),
        Line::from(vec![Span::styled(
            format!("{}% of total", code_percentage),
            Style::default().fg(Color::DarkGray),
        )]),
    ];
...
pub fn render_progress_bars(f: &mut ratatui::Frame, area: Rect, stats: &CodeStats) {
    ...
    let code_pct = if total_lines > 0.0 {
        (stats.total_code_lines as f64 / total_lines) * 100.0
    } else {
        0.0
    };
    ...
    let doc_gauge = Gauge::default()
        ...
        .label(format!(
            "{} / {} lines",
            stats.total_doc_lines, stats.total_lines
        ));
    f.render_widget(doc_gauge, chunks[2]);

    // Blank lines percentage with enhanced styling
    let blank_pct = if total_lines > 0.0 {
        (stats.total_blank_lines as f64 / total_lines) * 100.0
    } else {
        0.0
    };
    let blank_gauge = Gauge::default()
        ...
        .label(format!(
            "{} / {} lines",
            stats.total_blank_lines, stats.total_lines
        ));
    ...
    let chart_lines = if chart_data.is_empty() {
        ...
        for (language_name, language_info, line_count) in chart_data.iter().take(8) {
            ...
            lines.push(Line::from(vec![
                Span::styled(
                    format!("{} ", language_info.icon),
                    Style::default().fg(color),
                ),
                Span::styled(
                    format!("{:<12}", language_name),
                    Style::default()
                        .fg(Color::White)
                        .add_modifier(Modifier::BOLD),
                ),
...
pub fn render_export(f: &mut ratatui::Frame, area: Rect, app: &InteractiveApp) {
    ...
    let title_text = vec![
        ...
        Line::from(""),
        Line::from(vec![Span::styled(
            "Export your code analysis results to various formats",
            Style::default().fg(Color::Gray),
        )]),
        Line::from(vec![Span::styled(
            "Use ↑/↓ to select format, Enter to export, or press the number key",
            Style::default().fg(Color::Gray),
        )]),
    ];
    ...
    let status_text = vec![
        Line::from(vec![
            ...
            Span::styled(
                &app.export_state.export_status,
