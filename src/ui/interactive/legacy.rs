...
impl InteractiveDisplay {
    ...
    pub fn show_welcome(&mut self) -> io::Result<()> {
        if let Some(ref mut display) = self.modern_display {
            ...
            println!(
                "{}",
                "Intelligent code counting with beautiful visualization".bright_blue()
            );
            println!();
        }
        Ok(())
    ...
    fn show_fallback_results(
        ...
        println!(
            "📁 Total Files: {}",
            stats.total_files.to_string().bright_yellow()
        );
        println!(
            "📏 Total Lines: {}",
            stats.total_lines.to_string().bright_blue()
        );
        println!(
            "💻 Code Lines: {}",
            stats.total_code_lines.to_string().bright_green()
        );
        println!(
            "💬 Comment Lines: {}",
            stats.total_comment_lines.to_string().bright_magenta()
        );
        println!(
            "📚 Documentation Lines: {}",
            stats.total_doc_lines.to_string().bright_cyan()
        );
        println!(
            "⬜ Blank Lines: {}",
            stats.total_blank_lines.to_string().bright_black()
        );
        println!(
            "💾 Total Size: {}",
            Self::format_size_fallback(stats.total_size).bright_cyan()
        );
