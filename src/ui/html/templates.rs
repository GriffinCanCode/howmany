...
impl TemplateGenerator {
    ...
    pub fn with_sherlock_result(
        mut self,
        sherlock_result: crate::core::detector::SherlockResult,
    ) -> Self {
    ...
    pub fn generate_extension_rows_with_real_analysis(
        &self,
        ...
        for (ext, ext_stats) in extensions {
            ...
            let complexity_score = complexity_data
    ...
    pub fn generate_individual_files_section(
        &self,
    ...
    pub fn generate_optimized_individual_files_section(
        &self,
        individual_files: &[(String, FileStats)],
    ) -> String {
        if individual_files.is_empty() {
    ...
    pub fn generate_complexity_data_with_real_analysis(
        &self,
        aggregated_stats: &AggregatedStats,
    ) -> String {
        let mut data: Vec<String> = Vec::new();
        ...
        for (ext, _) in extensions {
            let complexity = aggregated_stats
                .complexity
    ...
    pub fn generate_complexity_insights(&self, stats: &BasicStats) -> String {
        ...
        if code_ratio > 0.8 {
            insights.push(
                "🔴 High code density detected. Consider adding more comments and documentation."
                    .to_string(),
            );
        } else if code_ratio > 0.6 {
            insights.push(
                "🟡 Moderate code density. Good balance but could benefit from more documentation."
                    .to_string(),
            );
        } else {
        ...
        if doc_ratio > 0.15 {
            insights.push(
                "📚 Excellent documentation coverage. Your future self will thank you!".to_string(),
            );
        } else if doc_ratio > 0.08 {
            insights.push(
                "📖 Good documentation coverage. Consider adding more for complex functions."
                    .to_string(),
            );
        } else if doc_ratio > 0.03 {
            insights.push(
                "📝 Basic documentation present. Consider expanding for better maintainability."
                    .to_string(),
            );
        } else {
            insights.push(
                "⚠️ Low documentation coverage. Adding docs will improve code maintainability."
                    .to_string(),
            );
        }
        ...
        if avg_file_size > 50000.0 {
            insights.push(
                "📏 Large average file size detected. Consider breaking down large files."
                    .to_string(),
            );
        } else if avg_file_size > 20000.0 {
            insights.push(
                "📐 Moderate file sizes. Monitor for files that might need refactoring."
                    .to_string(),
            );
        } else {
            insights
                .push("📋 Good file size distribution. Easy to navigate and maintain.".to_string());
        }
    ...
    pub fn generate_real_complexity_insights(&self, aggregated_stats: &AggregatedStats) -> String {
        ...
        if complexity_stats.function_count > 0 {
            ...
            if avg_complexity > 15.0 {
                ...
                insights.push(
                    "🟡 Moderate complexity. Monitor for functions that might need simplification."
                        .to_string(),
                );
            } else {
                insights.push(
                    "🟢 Good complexity levels. Functions are well-structured and maintainable."
                        .to_string(),
                );
            }
            ...
            if complexity_stats.max_nesting_depth > 6 {
                ...
                insights.push(
                    "📏 Moderate nesting levels. Keep an eye on deeply nested code.".to_string(),
                );
            } else {
                insights.push(
                    "📋 Good nesting levels. Code structure is clean and readable.".to_string(),
                );
            }
            ...
            if complexity_stats.average_function_length > 50.0 {
                insights.push(
                    "📏 Large average function size. Consider breaking down large functions."
                        .to_string(),
                );
                ...
                insights.push(
                    "📋 Good function size distribution. Functions are focused and manageable."
                        .to_string(),
                );
            }
        }
        ...
        if quality.code_health_score > 80.0 {
            insights.push(
                "⭐ Excellent code quality! Your codebase is well-structured and maintainable."
                    .to_string(),
            );
        } else if quality.code_health_score > 60.0 {
            insights.push(
                "👍 Good code quality. Some areas could benefit from improvement.".to_string(),
            );
        } else {
            insights.push(
                "⚠️ Code quality needs attention. Consider refactoring and adding documentation."
                    .to_string(),
            );
        }
    ...
    pub fn generate_quality_recommendations(&self, stats: &CodeStats) -> String {
        ...
        if comment_ratio < 0.1 {
            recommendations.push(
                "💬 Add more inline comments to explain complex logic and business rules."
                    .to_string(),
            );
        }
        ...
        if doc_ratio < 0.05 {
            recommendations.push(
                "📚 Add function and class documentation to improve code understanding."
                    .to_string(),
            );
        }
        ...
        if stats.total_files > 1000 {
            recommendations.push(
                "📁 Consider organizing files into modules or packages for better structure."
                    .to_string(),
            );
        }

        // Testing recommendations
        let has_test_files = stats
            .stats_by_extension
            .keys()
        ...
        if !has_test_files {
            recommendations.push(
                "🧪 Add unit tests to improve code reliability and maintainability.".to_string(),
            );
        }
    ...
    pub fn generate_real_quality_recommendations(
        &self,
        aggregated_stats: &AggregatedStats,
    ) -> String {
        let mut recommendations = Vec::new();
        ...
        if complexity_stats.cyclomatic_complexity > 15.0 {
            recommendations.push(
                "🔧 Refactor high-complexity functions to improve maintainability.".to_string(),
            );
        }

        if complexity_stats.max_nesting_depth > 6 {
            recommendations.push(
                "📐 Reduce nesting depth by extracting logic into separate functions.".to_string(),
            );
        }

        if complexity_stats.average_function_length > 50.0 {
            recommendations
                .push("✂️ Break down large functions into smaller, focused units.".to_string());
        }

        if complexity_stats.average_parameters_per_function > 5.0 {
            recommendations.push(
                "📝 Consider using objects or structs to group related parameters.".to_string(),
            );
        }
        ...
        if quality.maintainability_index < 70.0 {
            recommendations.push(
                "🔧 Focus on improving maintainability through better structure and documentation."
                    .to_string(),
            );
        }
        ...
        if quality.function_size_health < 70.0 {
            recommendations
                .push("📖 Improve code readability by breaking down large functions.".to_string());
        }

        if quality.nesting_depth_health < 70.0 {
            recommendations.push(
                "🧪 Reduce nesting depth to improve testability and readability.".to_string(),
            );
        }

        if quality.documentation_coverage < 10.0 {
            recommendations.push(
                "💬 Add more comments to explain complex logic and business rules.".to_string(),
            );
        }
    ...
    pub fn generate_enhanced_insights(&self, aggregated_stats: &AggregatedStats) -> String {
        ...
        if complexity_stats.function_count > 0 {
            ...
            if avg_complexity > 15.0 {
                insights.push(
                    "🔴 High complexity detected - consider refactoring for better maintainability"
                        .to_string(),
                );
            } else if avg_complexity > 10.0 {
                insights.push(
                    "🟡 Moderate complexity - monitor for potential simplification opportunities"
                        .to_string(),
                );
            } else {
                insights.push(
                    "🟢 Good complexity levels - well-structured and maintainable code".to_string(),
                );
            }
        }
        ...
        if doc_ratio > 0.2 {
            insights.push(
                "📚 Excellent documentation coverage - future developers will appreciate this"
                    .to_string(),
            );
        } else if doc_ratio > 0.1 {
            insights.push(
                "📖 Good documentation coverage - consider expanding for complex areas".to_string(),
            );
        } else {
            insights.push(
                "📝 Limited documentation - adding docs will improve maintainability".to_string(),
            );
        }
        ...
        if basic_stats.total_lines > 10000 {
            insights
                .push("📁 Large codebase - consider modular organization strategies".to_string());
        } else if basic_stats.total_lines > 1000 {
            insights.push(
                "📂 Well-sized project - good balance of organization and complexity".to_string(),
            );
        } else {
    ...
    pub fn generate_enhanced_recommendations(&self, aggregated_stats: &AggregatedStats) -> String {
        ...
        if quality.code_health_score < 60.0 {
            ...
            recommendations.push(
                "⚠️ Code health could be improved - consider incremental refactoring".to_string(),
            );
        }
        ...
        if complexity_stats.max_nesting_depth > 4 {
            recommendations
                .push("📐 Reduce nesting depth using early returns and guard clauses".to_string());
        }

        if ratios.comment_ratio < 0.1 {
            recommendations.push(
                "💬 Add inline comments to explain business logic and complex algorithms"
                    .to_string(),
            );
        }
        ...
        if ratios.doc_ratio < 0.05 {
            recommendations
                .push("📚 Add API documentation for public functions and classes".to_string());
        }

        if basic_stats.average_lines_per_file > 500.0 {
            recommendations
                .push("📄 Break down large files into smaller, focused modules".to_string());
        }

        // Testing recommendations
        let has_tests = basic_stats
            .stats_by_extension
            .keys()
            .any(|ext| ext.contains("test") || ext.contains("spec"));
    ...
    pub fn generate_enhanced_individual_files_section(
        &self,
        individual_files: &[(String, FileStats)],
    ) -> String {
        if individual_files.is_empty() {
        ...
        for (file_path, file_stats) in sorted_files.iter().take(20) {
            ...
            let complexity_class = self.get_complexity_class_for_score(complexity);
            let functions = self.calculate_real_function_count(file_path, file_stats);
            let risk_level = if complexity > 15.0 {
                "HIGH"
    ...
    fn generate_language_insights(&self, stats: &BasicStats) -> Vec<String> {
        ...
        if let Some((most_used_ext, most_used_stats)) = stats
            .stats_by_extension
            .iter()
            .max_by_key(|(_, ext_stats)| ext_stats.total_lines)
        {
            let percentage =
                (most_used_stats.total_lines as f64 / stats.total_lines as f64) * 100.0;

            if percentage > 80.0 {
                insights.push(format!(
                    "🎯 Primarily {} codebase ({:.1}%). Consider language-specific best practices.",
                    self.get_language_name(most_used_ext),
                    percentage
                ));
            } else if percentage > 60.0 {
                insights.push(format!(
                    "🌟 Mainly {} with some diversity ({:.1}%). Good balance of technologies.",
                    self.get_language_name(most_used_ext),
                    percentage
                ));
            }
        ...
        if language_count > 10 {
            insights.push(
                "🌐 Highly polyglot project. Ensure consistent coding standards across languages."
                    .to_string(),
            );
