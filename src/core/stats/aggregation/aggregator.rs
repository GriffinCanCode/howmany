...
use super::types::{AggregatedStats, AnalysisDepth, StatsMetadata};
...
impl StatsAggregator {
    ...
    pub fn get_summary(&self, stats: &AggregatedStats) -> BTreeMap<String, String> {
        ...
        summary.insert(
            ...
            stats.basic.total_files.to_string(),
        );
        summary.insert(
            "total_lines".to_string(),
            stats.basic.total_lines.to_string(),
        );
        summary.insert("code_lines".to_string(), stats.basic.code_lines.to_string());
        summary.insert(
            "functions".to_string(),
            stats.complexity.function_count.to_string(),
        );
        summary.insert(
            "avg_complexity".to_string(),
            format!("{:.1}", stats.complexity.cyclomatic_complexity),
        );
        summary.insert(
            "quality_score".to_string(),
            format!("{:.1}", stats.ratios.quality_metrics.overall_quality_score),
        );
        summary.insert(
            "languages".to_string(),
