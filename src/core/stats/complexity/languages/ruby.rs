...
impl RubyAnalyzer {
    ...
    fn count_cognitive_complexity(&self, line: &str, nesting_level: usize) -> usize {
        ...
        if line.contains(".each") || line.contains(".map") || line.contains(".select") {
            complexity += 1;
        }
        if line.contains("lambda") || line.contains("proc") || line.contains("->") {
            complexity += 1;
        }
        if line.contains("eval") {
            complexity += 3;
        } // eval is very complex
        if line.contains("define_method") || line.contains("method_missing") {
            complexity += 2;
        }

        // Metaprogramming adds complexity
        if line.contains("class_eval")
            || line.contains("instance_eval")
            || line.contains("module_eval")
        {
            complexity += 2;
        }
    ...
    fn is_structure_declaration(&self, line: &str) -> bool {
        ...
        structure_keywords
            .iter()
            .any(|&keyword| trimmed.starts_with(keyword) && !trimmed.starts_with('#'))
    }
        ...
        for (line_num, line) in lines.iter().enumerate() {
            ...
            if in_structure {
                ...
                if let Some(ref mut structure) = current_structure {
                    ...
                    if trimmed.contains("attr_reader")
                        || trimmed.contains("attr_writer")
