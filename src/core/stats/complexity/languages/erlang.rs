...
impl ErlangAnalyzer {
    ...
    fn extract_behavior_name(&self, line: &str) -> Option<String> {
        ...
        if (trimmed.starts_with("-behaviour(") || trimmed.starts_with("-behavior("))
            && trimmed.ends_with(").")
        {
            let start_pos = if trimmed.starts_with("-behaviour(") {
                11
            } else {
                10
            };
    ...
    fn count_cognitive_complexity(&self, line: &str, nesting_level: usize) -> usize {
        ...
        if line.contains("->") && !line.contains("fun") {
            complexity += 1;
        }
        ...
        if line.contains("spawn_link(") {
            complexity += 1;
        }
        if line.contains("spawn_monitor(") {
            complexity += 1;
        }
