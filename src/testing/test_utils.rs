...
impl TestProject {
    ...
    pub fn create_rust_file(
        ...
        for i in 0..functions {
            ...
            content.push_str(&format!(
                "/// This function demonstrates complexity level {}\n",
                i % 3 + 1
            ));
            content.push_str(&format!(
                "pub fn function_{}(param: i32) -> Result<String> {{\n",
                i
            ));
    ...
    pub fn create_python_file(&self, path: &str, functions: usize) -> Result<PathBuf> {
        ...
        for i in 0..functions {
            content.push_str(&format!(
                "def function_{}(param: int) -> Optional[str]:\n",
                i
            ));
            content.push_str(&format!("    \"\"\"\n    Function {} docstring\n", i));
            content.push_str("    \n");
    ...
    pub fn create_typescript_file(&self, path: &str, functions: usize) -> Result<PathBuf> {
        ...
        for i in 0..functions {
            ...
            content.push_str(" */\n");
            content.push_str(&format!(
                "export async function function_{}(param: number): Promise<ProcessResult> {{\n",
                i
            ));
            ...
            match i % 2 {
                ...
                _ => {
                    ...
                    content.push_str("        for (let i = 0; i < param; i++) {\n");
                    content.push_str(
                        "            items.push({ id: i, name: `Item ${i}`, value: i * 2 });\n",
                    );
                    content.push_str("        }\n");
                    content.push_str("        return {\n");
                    content.push_str("            success: true,\n");
...
"#,
    ...
    pub fn create_target_dir(&self) -> Result<()> {
        ...
        self.create_file(
            "target/debug/myapp.d",
            "target/debug/myapp: src/main.rs src/lib.rs",
        )?;
        self.create_file(
            "target/release/myapp.d",
            "target/release/myapp: src/main.rs src/lib.rs",
        )?;

        // Create some dependency artifacts
        self.create_file_binary(
            "target/debug/deps/libserde-123.rlib",
            &[0x72, 0x6c, 0x69, 0x62],
        )?;
        self.create_file_binary(
            "target/release/deps/libserde-456.rlib",
            &[0x72, 0x6c, 0x69, 0x62],
        )?;
    ...
    pub fn create_python_cache(&self) -> Result<()> {
        ...
        self.create_file_binary(
            "tests/__pycache__/test_main.cpython-39.pyc",
            &[0x61, 0x0d, 0x0d, 0x0a],
        )?;
